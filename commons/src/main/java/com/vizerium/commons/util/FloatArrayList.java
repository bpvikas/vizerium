...
public class FloatArrayList {
	...
	public FloatArrayList(int initialCapacity) {
		data = new float[Math.max(initialCapacity, 10)];
		size = 0;
	...
	/**
	 * Returns the index of the element in the internal array that is {@code offset} positions away from the element at the index {@code index}, wrapping around the array if
	 * necessary. {@code offset} can be negative, in which case the distance will be traveled to the left of the original element rather than to the right. This method assumes that
	 * the passed index is valid and that the absolute value of {@code offset} is not greater than the length of the internal array. If these conditions are not met, the returned
	 * index might not be valid.
	 *
	 * @param index
	 *            a valid index for the internal array
	 * @param offset
	 *            the distance from the element at index {@code index}; may be negative
	...
	/**
	 * Converts an index for this {@code IntArrayList} to the corresponding index for the internal array. This method assumes that the passed index is valid. If it is not, the
	 * returned index might not be valid as well.
	 *
	 * @param publicIndex
	 *            A valid index for this {@code IntArrayList}
	...
	public int hashCode() {
		...
		for (int i = 0; i < size; i++) {
			hashCode = 31 * hashCode + Float.hashCode(get(i));
		}
		return hashCode;
	}

	// moves a range without wrapping around
	private static void moveRange(float[] array, int sourceStartPosition, int destinationStartPosition, int length) {
		if (sourceStartPosition < 0 || destinationStartPosition < 0 || length < 0 || sourceStartPosition > array.length - length
	...
	private static void moveRangeWrapping(float[] array, int sourceStartPosition, int offset, int length) {
		...
		if (offset > 0) {
			if (sourceStartPosition > array.length - length - offset) {
				...
				moveRange(array, tempStartPosition, tempStartPosition + offset - array.length, (sourceStartPosition < array.length - length ? sourceStartPosition + length
						: array.length) - tempStartPosition);
			}
